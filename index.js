...
function reset() {
  ...
  boards(board_Player1);
  boards(board_Player2);
  document.getElementById("ships_player1").textContent = player1.shipCount;
  document.getElementById("ships_player2").textContent = player2.shipCount;
  document.getElementById("turn_player").textContent = currentplayer.name;
}
function gameWinner() {
  var winner = player1.shipCount === 0 ? player2.name : player1.name;
  alert("The winner of this game is PLAYER " + winner + " THE GAME IS OVER");
  return winner;
}

function boards(playerBoard) {
  //const board_Player1 = document.getElementById('board_player1');
  for (var x = 0; x < 4; x++) {
    ...
    for (var y = 0; y < 4; y++) {
      ...
      cell.addEventListener("click", (e) => {
        ...
        //Check if the square belongs to our opponents board
        let boardElement = cell.parentNode.parentNode;
        if (currentplayer == player1 && boardElement == board_Player1) {
          return;
        } else if (currentplayer == player2 && boardElement == board_Player2) {
          return;
        }
        ...
        //Check if the square is the ship
        let coord = cell.textContent.split(",");
        //console.log(coordp1,"array")
        var x = parseInt(coord[0]);
        var y = parseInt(coord[1]);
        disparar(opponent, x, y); // player2 shot player 1 in player1 board
        ...
        //Change the current player to be the opponent
    ...
    playerBoard.appendChild(li); //adding each row into the board
  }
} //end function boards
createButtons();
createShip(player1, 4);
